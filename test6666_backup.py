...
class EHLSolver:
    def __init__(self, lift_file="updated_lift.txt"):
        ...
        # tau_limit = tau_0 + gamma_L * P, but capped at tau_max
        # Lower gamma_L means less increase of limiting shear with pressure
        # This gives more limiting at high pressures (nose region)
        # ------------------------
        self.tau_0 = 4.0e6       # Pa - base limiting shear stress
        self.gamma_L = 0.02     # lower value for stronger limiting at high pressure
        self.tau_max = 20.0e6   # Pa - maximum limiting shear stress
    ...
    # Based on standard EHL formulation (Venner & Lubrecht, Hamrock & Dowson)
    # ------------------------
    def system_func(self, V):
        ...
        # Load balance: integral = ∫P_tot dx (non-dimensional)
        integral = np.sum(P_tot * self.dx)
        
        # Normalized load balance residual
        if self.Wld > 1e-12:
            F_load = (self.Wld - integral) / self.Wld
        else:
            F_load = self.Wld - integral
    ...
    def build_initial_guess(self):
        ...
        # Hamrock-Dowson line contact formula for minimum film thickness
    ...
    # References:
    #   - Bair & Winer (1979), Johnson & Tevaarwerk (1977) - Eyring limiting shear stress
    #   - Standard EHL friction: tau = tau_L * arcsinh(tau_N / tau_L)
    # ------------------------
    def compute_friction_and_torque(self, P_rey_nd, Pa_nd, H_nd, lift_i):
        """
        Hydrodynamic wall shear with Eyring limiting shear stress model.
        No artificial smoothing or weighting - pure physics formulation.
        """
        ...
        # Pressure gradient dp/dx (Pa/m) - standard central difference
        dpdx = np.gradient(p_rey, dx_m)

        # Local viscosity (Pa*s)
        mu = self.c_mu(P_rey_nd, self.T_current)

        # Flow factors (Patir-Cheng for rough surfaces)
        phi_x, phi_s = self.calc_flow_factors(H_nd)

        # ===== LIMITING SHEAR STRESS MODEL =====
        # Eyring/Johnson-Tevaarwerk limiting shear stress
        tau_limit = np.minimum(self.tau_0 + self.gamma_L * p_rey, self.tau_max)
        
        # Newtonian shear stress components
