...
def asperity_pressure_zhao_masjedi(
    ...
    r = float(sigma)                     # RMS roughness [m]
    hd = float(hardness_GPa) * 1e9       # hardness [Pa]

    # Effective line load (per unit width)
    W_total = float(max(W_total, 0.0))
    if W_total <= 0.0:
        return np.zeros_like(h)

    # ------------ dimensionless groups (Masjedi 2012) ------------
    # W̄, Ū, Ḡ, r̂, V̄
    R_eff = float(max(R, 1e-12))
    w_line = W_total / max(L, 1e-30)          # [N/m]

    W_dim = w_line / (E_star * R_eff + 1e-30)          # W̄
    U_dim = eta0 * abs(Ve) / (E_star * R_eff + 1e-30)  # Ū
    G_dim = alpha0 * E_star                            # Ḡ
    r_hat = r / (R_eff + 1e-30)                        # r̂
    V_dim = hd / (E_star + 1e-30)                      # V̄

    # clamp to avoid underflow / overflow in powers and logs
    W_cl   = max(W_dim, 1e-12)
    U_cl   = max(U_dim, 1e-12)
    G_cl   = max(G_dim, 1e-12)
    rhat_cl = max(r_hat, 1e-12)
    V_cl   = max(V_dim, 1e-12)

    # ------------ Masjedi asperity load ratio L_a (curve-fit to Zhao model) ------------
    # Functional form from Masjedi & Khonsari (2012, asperity-load formula):
    #   L_a = C * W̄^a * Ū^b * Ḡ^c *
    #         ln[ 1 + K * r̂^{m} * V̄^{n} * W̄^{p} * Ū^{q} * Ḡ^{s} ]
    #
    # Coefficients (C, a, b, c, K, m, n, p, q, s) taken from the published fit.
    # (These numbers are specific to *line contacts* and Gaussian roughness.)
    C  = 5.0e-3
    a  = -0.408
    b  = -0.088
    c  =  0.103
    ...
    # numeric safety: enforce >1 before log
    inner = 1.0 + K * (rhat_cl**m) * (V_cl**n) * (W_cl**p) * (U_cl**q) * (G_cl**s)
    if inner <= 1.0:
        La_frac = 0.0
    else:
    # La from Eq. (21) is in **percent**
        La_percent = C * (W_cl**a) * (U_cl**b) * (G_cl**c) * np.log(inner)
        La_percent = max(La_percent, 0.0)
    # convert percent → fraction of total load
        La_frac = La_percent / 100.0

# optional: avoid unphysical >70% asperity load (Masjedi advises caution there)
    La_frac = min(La_frac, 0.7)

    Wa_target = La_frac * W_total

    if Wa_target <= 0.0:
        return np.zeros_like(h)

    # ------------ spatial distribution via Greenwood–Tripp ------------
    # use your existing F_{3/2}(λ) lookup: _lam_grid, _F32_lookup
    lam = np.maximum(h / (r + 1e-30), 0.0)
    lam_clipped = np.clip(lam, _lam_grid[0], _lam_grid[-1])

    F32 = np.interp(lam_clipped, _lam_grid, _F32_lookup)
    F32 = np.maximum(F32, 0.0)

    if not np.any(F32 > 0.0):
        return np.zeros_like(h)

    # scale F32 so that ∫ p_asp dx * L = Wa_target
    denom = np.trapezoid(F32, x) * L
    if denom <= 1e-30:
        return np.zeros_like(h)

    scale = Wa_target / denom        # [Pa], because Wa = ∫(scale*F32)dx * L
    p_asp_x = scale * F32

    # numeric safety: no negative pressures due to roundoff
...
def eta_houpert(p, T0_c, Ve_local, h_local, a_local):
    Z, S0, lneta0_plus = _houpert_params(
        eta0, T0_c, alpha0, BETA0_CONST
    )
    # First pass at T0_c
...
def htex_profile(x, a_theta, atex_theta, shift_theta, d_texture):
    ...
    h = E_eff*atex_theta * np.exp(expo)
...
def solve_theta(R, Ve, Vs, W, dt, angle_deg, rpm,
    ...
    for _sub in range(total_substeps):
        ...
        np.maximum(h_nom, h_floor), x, W, Ve, R
        )
...
# Mixed load split: Zhao/Masjedi asperity + hydrodynamic (no extra scaling)
    ...
    hardness_GPa=7.0          # case-hardened 16MnCr5 / 100Cr6
   )
...
def _compute_sweep_theta(rpm, textured, dens_keys, sigma_values, var_key):
    ...
    if sigma_values is None or len(_as_list(sigma_values)) == 0:
        if not textured and var_key in ("Fh", "Fb", "Wa"):
            file_kind = {"Fh": "Fh_theta", "Fb": "Fb_theta", "Wa": "Wa_theta"}[var_key]
...
def plot_kinematic_variables():
    ...
    for rpm in rpms:
        ...
        nvar = len(vars_and_labels)
        for k, (y, lab) in enumerate(vars_and_labels, start=1):
            plt.subplot(ceil(nvar / 2), 2, k)
            plt.plot(TH_DEG, y)
            plt.xlabel("Cam angle (deg)")
            plt.ylabel(lab)
            plt.grid(True, alpha=0.3)
        plt.suptitle(f"Kinematic / contact variables vs angle — rpm={rpm}")
