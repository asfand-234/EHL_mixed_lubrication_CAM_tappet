...
TASK:
Your task is to create a fitted polynomial equation that gives upto 85% accurcay. 
Try to make less order polynomial by using support of other mathematical functions like exp, log, powers, sin/cos wherever helpful.

HARD STRICT RULES:

1)  Prioritize interpretability and simplicity but maintain perfect accuracy.
2) The accuracy must be upto 85%
3) Do not only check the overall R^2 but also check error between predicted E and original E. If error greater than keep optimizing.
TESTING CRITERIA:
