...
1) Start first tuning Flash Temperature parameters for 90°C only and for only 5% texture area density and for 300 RPM. Understand the effect of each parameter.
 Once fitted then these must be constant for all RPMs.
...
MASK_THR_MULT =             [3-12]
2) When you see the results are slightly better then start tuning SGTS gate parameters. When these once fitted then these Must remain constant for all RPMs.
...
4) If the results are not 80% close to the target values then re-start again from tuning flash temperature parameter and keep repeating all above 3 steps until you get at least 80% same target results.
